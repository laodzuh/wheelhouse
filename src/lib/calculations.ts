...
export function calculateOptionPnL(trade: Trade): number | null {
  ...
  if (action === "Sell to Open") {
    ...
    if (status === "Expired") return credit;
...
export function calculateDashboardStats(trades: Trade[], accountSize: number = 0): DashboardStats {
  ...
  return {
    ...
    avgWin: wins.length > 0 ? wins.reduce((s, w) => s + w, 0) / wins.length : 0,
    avgLoss:
      losses.length > 0
        ? losses.reduce((s, l) => s + l, 0) / losses.length
        : 0,
